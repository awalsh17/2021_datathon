...
```
# Suggestion: additional filtering to reduce data size
If all the plots will involve selecting a judge, then we are not using the rows without a Judge.
We can pre-filter out those rows and make operations on the data faster.

```{r}
filter_completerecords <- completerecords %>% 
  filter(Judge != "NA NA")
saveRDS(filter_completerecords, file = "new_bail_data.rds")
```
